    ...
    with open(template_jmx) as temp_stream:
        ...
        with open(os.path.join(current_dir, new_jmx_path), 'w') as new_stream:
            for line in lines:
                ...
                if setHost:
                    new_line = new_line.replace('<stringProp name="HTTPSampler.domain">www.baidu.com</stringProp>',
...
def runJmeterByCmd(new_jmx_dir, hostname='', ip=''):
    ...
    if hostname:
        execute_cmd = 'jmeter -Jurl={1} -n -t {0}.jmx -l {0}.jtl -j {0}.log -f -e -o {0}/result/'.format(new_jmx_dir,
                                                                                                         hostname)
    else:
